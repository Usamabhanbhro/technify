...
const forgotPassword = async (req, res) => {
  try {
    ...
    // Send reset email
    const resetUrl = `${process.env.FRONTEND_URL || 'http://localhost:5173'}/reset-password?token=${resetToken}&type=teacher`;
...
const verifyResetToken = async (req, res) => {
  try {
    ...
    // Find teacher and verify token matches and hasn't expired
    const teacher = await Teacher.findById(decoded.teacherId);

    if (!teacher || teacher.resetPasswordToken !== token) {
      return res.status(400).json({
        success: false,
    ...
    if (new Date() > teacher.resetPasswordExpiry) {
      return res.status(400).json({
        success: false,
        message: 'Reset token has expired. Please request a new one.'
      });
