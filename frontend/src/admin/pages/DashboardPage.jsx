...
const DashboardPage = () => {
  ...
  return (
    <div className="space-y-8 bg-gray-50 p-6 rounded-2xl">
      ...
      {/* Student Distribution with Chart */}
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-gray-800 mb-6">Comprehensive Metrics Overview</h2>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart
            data={[
              {
                name: 'Students',
                Total: stats.analytics.totalEnrollment,
                Enrolled: stats.analytics.currentEnrollment,
                Dropout: stats.analytics.totalDropout,
                Passout: stats.analytics.totalPassout
              }
            ]}
          ...
          >
            ...
            <Bar dataKey="Dropout" fill="#ef4444" />
            <Bar dataKey="Passout" fill="#8b5cf6" />
          </BarChart>
        </ResponsiveContainer>
      </div>

      {/* Charts Section - Fee & Attendance */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Fee Analytics Chart */}
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          <h2 className="text-xl font-bold text-gray-800 mb-6">Fee Collection Overview</h2>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie
                data={[
                  { name: 'Paid', value: stats.finance.paidCount },
                  { name: 'Pending', value: stats.finance.unpaidCount }
                ]}
                cx="50%"
                cy="50%"
                labelLine={false}
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="p-3 bg-green-50 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-green-600">Rs. {(stats.finance.paidAmount / 1000).toFixed(1)}K</p>
            </div>
            <div className="p-3 bg-yellow-50 rounded-lg text-center">
              <p className="text-sm text-gray-600">Total Pending</p>
              <p className="text-2xl font-bold text-yellow-600">Rs. {(stats.finance.unpaidAmount / 1000).toFixed(1)}K</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={[
                {
                  name: 'Attendance',
                  Present: stats.attendance.presentCount,
                  Absent: stats.attendance.absentCount
                }
              ]}
              margin={{ top: 20, right: 30, left: 0, bottom: 20 }}
            >
          ...
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div className="p-3 bg-blue-50 rounded-lg text-center">
              ...
              <p className="text-2xl font-bold text-blue-600">{stats.attendance.presentPercentage}%</p>
            </div>
            <div className="p-3 bg-red-50 rounded-lg text-center">
              <p className="text-sm text-gray-600">Absent</p>
              <p className="text-2xl font-bold text-red-600">{stats.attendance.absentPercentage}%</p>
            </div>
          </div>
        </div>
      </div>
